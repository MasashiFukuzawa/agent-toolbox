...
ユーザーは複数企業で副業しており、企業Aの作業が企業Bに混入することは情報漏洩リスクに直結する。
...
`/done` を呼び出し、**出力末尾の `quality-gate: PASS` 署名のみを成功判定の根拠**とする。
...
### Step 8: Merge（tier 問わず自律・許可不要）

CI が緑であれば tier に関係なく自律で merge する。
Step 7 で CI green を確認済みなので `--auto` は不要。merge 戦略フラグは排他なので単一で指定する:
...
- **`/done` の内部実装（tier/step）に依存しない。`quality-gate: PASS` 署名のみ見る**
