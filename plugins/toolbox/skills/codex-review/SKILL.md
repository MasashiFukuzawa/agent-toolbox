...
```bash
# 未コミットの変更をレビュー
codex exec review -m gpt-5.5 -c model_reasoning_effort="xhigh" --uncommitted < /dev/null

# 特定のブランチとの差分をレビュー
codex exec review -m gpt-5.5 -c model_reasoning_effort="xhigh" --base main < /dev/null

# 特定コミットをレビュー
codex exec review -m gpt-5.5 -c model_reasoning_effort="xhigh" --commit COMMIT_SHA < /dev/null
```
...
3. 修正後は元の検査に加え、secret/confusable等の関連scanを再実行する。レビュー修正そのものが新しい欠陥を作り得る。
